...
public class SignArguments : ISignArguments
{
    ...
    public SigningProviderType SigningProvider { get; set; } = SigningProviderType.FileName;

    [Option('k', "keyfilename", Required = false, Default = null, HelpText = "FileName for Private Key")]
    public string? PrivateKeyFileName { get; set; }

    [Option('v', "keyvault", Required = false, Default = null, HelpText = "Azure KeyVault connection string (Url=;KeyName=;[KeyVersion=;TenantId=;ClientId=;ClientSecret=;])")]
    ...
    {
        if (SigningProvider == SigningProviderType.FileName && string.IsNullOrWhiteSpace(PrivateKeyFileName))
            throw new CommandLineParserException($"Must specify {nameof(PrivateKeyFileName)} for {nameof(SigningProvider)} {SigningProvider}");

        if (SigningProvider == SigningProviderType.AzureKeyVault && string.IsNullOrWhiteSpace(AzureKeyVaultConnectionString))
