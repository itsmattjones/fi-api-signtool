...
internal class Program
{
    ...
    private static int RunSign(ISignArguments arguments, RunResult runResult)
    {
        ...
        var inputData = runResult.HashedData?.Length > 0 ? runResult.HashedData : ByteArrayHelper.TranslateByteArray(arguments.InputData);
    ...
    private static int RunEncode(IEncodeArguments arguments, RunResult runResult)
    {
        ...
        var inputData = runResult.SignedData?.Length > 0 ? runResult.SignedData : ByteArrayHelper.TranslateByteArray(arguments.InputData);
        Console.Out.WriteLine($"Using: {ByteArrayHelper.DescribeByteArray(inputData)}");
